...
export class TaskFilterComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.filterForm.valueChanges
      .pipe(
        ...
        switchMap(formValues => this.store.select(selectAllTasks).pipe(
  ...
  private applyFilter(tasks: Task[], formValues: TaskFilterForm): Task[] {
    ...
    const matchingUserIDs = executorName ? this.allUsers
      .filter(user => user.username.toLowerCase().includes(executorName))
      .map(user => user.id) : [];
